...
total.inf.inc <- import(here("04_final_datasets", "infection_draws", "total.inc.per100k.draws.scen2.RDATA"))
...
uti.upper.cases <- import(here("04_final_datasets", "infection_draws", "uti.pred.upper.30perc.cases.RDATA")) %>% dplyr::select(-c(pop_totl)) # higher percent
severe.lri.cases <- import(here("04_final_datasets", "infection_draws", "severe.lri.high.cases.RDATA")) # high cases 
typhoid.cases <- import(here("04_final_datasets", "infection_draws", "typhoid.25xsuspected.10per100k.cases.RDATA"))
ssti.nf.cases <- import(here("04_final_datasets", "infection_draws", "high.ssti.cases.RDATA")) %>% rename_with(~ sub("^V", "X", .x), .cols = starts_with("V"))
iai.cases <- import(here("04_final_datasets", "infection_draws", "high.iai.cases.RDATA")) %>% rename_with(~ sub("^V", "X", .x), .cols = starts_with("V"))
hap.cases <- import(here("04_final_datasets", "infection_draws", "hap.cases.lriratio.RDATA")) 
