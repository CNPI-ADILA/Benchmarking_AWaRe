...
# Full MIDAS data - scenario 2 - sensitivity analyses - high Watch scenario
#######
...
# Scenario 2 details: this looks to establish a high Watch use scenario using the adjusted case counts in total (as scenario 1) 
# Uses total infection incidence as the inflated incidence
# Use Watch scenario for typhoid where it is inflated 25x where incidence >10 per 100k, 30.7% upper UTI, 20% severe LRI in HIC and 30% severe LRI in LMIC (clusters 1, 2, 3), + additional Watch for IAI); HAP stays the same
# this is still the main dataset & same benchmark countries as scenario 1 and 2 - this uses the same total incidence envelop - the main thing that changes are the case counts for watch infections thus Watch DDD changes 
# For typhoid use the azithromycin ddd of 15 
...
total.inf.inc <- import(here("data", "total.inc.per100k.draws.scen2.RDATA"))
...
uti.upper.cases <- import(here("data", "uti.pred.upper.30perc.cases.RDATA")) %>% dplyr::select(-c(pop_totl)) # higher percent
severe.lri.cases <- import(here("data", "severe.lri.high.cases.RDATA")) # high cases 
typhoid.cases <- import(here("data", "typhoid.25xsuspected.10per100k.cases.RDATA"))
ssti.nf.cases <- import(here("data", "high.ssti.cases.RDATA")) %>% rename_with(~ sub("^V", "X", .x), .cols = starts_with("V"))
iai.cases <- import(here("data", "high.iai.cases.RDATA")) %>% rename_with(~ sub("^V", "X", .x), .cols = starts_with("V"))
hap.cases <- import(here("data", "hap.cases.lriratio.RDATA")) 


# specify scenario 
scenario.folder <- "scenario_2"
scenario.file <- "scen2"
...
# - use the higher DDD for typhoid of 15 for azithromycin
...
typhoid.ddd <- typhoid.cases %>% mutate(across(-iso3_code, ~ . * 15)) # use the azithromycin DDD
