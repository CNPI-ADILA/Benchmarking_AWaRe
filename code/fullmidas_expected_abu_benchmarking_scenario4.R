...
# Full MIDAS data - scenario 4 - same scenario as the primary analysis using the 10x typhoid but with a diff benchmark country for groups 123 -  DIFFERENT INFLATION FACTOR FOR TYPHOID (10X INSTEAD OF 25X) 
#######
...
# Scenario 4 details: this looks to compare the primary analysis scenario of expected total and AWaRe precsribing when using a different benchmark country for groups 1, 2, 3
...
#     This is a sensitivty analysis where we inflate typoid at >50 per 100k but we inflate only 10x (e.g. 10 suspected cases needing treatment for every 1 confirmed) not 25x
# this is the main dataset & same benchmark countries (as scenarios 2 and 3) from MIDAS
# Looking at Colombia as alternate benchmark country for clusters 1, 2, 3

# *** UPDATED 01 APRIL 2026 - in response to reviewer comments LPH - excluded XDR TB from Watch ***
...
# infection incidence draws when including the estimates of HAIs that weren't in GBD - same as scenario 1
...
# -------------------------------------------------------------#
# Change scenario & benchmark info depending on criteria ####
#--------------------------------------------------------------#
# specify scenario 
scenario.folder <- "scenario_4"
scenario.file <- "scen4"


# specify benchmark countries 
benchmark.country.c4 <- "CHE"
benchmark.country.c123 <- "COL"
...
# Cluster 3 = Colombia
# Cluster 1 & 2 - no suitable benchmark so using cluster 3 benchmark = Colombia
...
vro.cases <- import(here("data", "vro.cases.df.RDATA"))
...
# # Loop over datasets, filter to just analysis countries & ensure correct order -  then sum up 
# for (i in seq_along(reserve_midas_cases_datasets)) {
#   data <- reserve_midas_cases_datasets[[i]] # select dataset
#   data_filtered <- data[data$iso3_code %in% analysis.countries, ] # filter to analysis countries
#   
#   # Ensure the order matches analysis_countries
#   data_filtered <- data_filtered %>%
#     arrange(match(iso3_code, analysis.countries))
#   
#   if (ncol(data_filtered) == 1001 && "iso3_code" %in% names(data_filtered)) {
#     data_filtered <- data_filtered[, -which(names(data_filtered) %in% c("iso3_code"))] # remove iso3_code, keep only numeric values for summing
#     summed_cases_midas[[i]] <- data_filtered # store filtered dataset 
#   } else {
#     warning(paste("Dataset", dataset_names[i], "does not have the correct format. Skipping."))
#   }
# }
...
# multiply ratio of GNB DDD : CRO by all cro cases to get expected DDD reserve 
midas.cro.reserve.ddd.exp <- cro.cases %>%
  filter(iso3_code %in% analysis.countries) %>% # Filter to just the analysis countries 
  arrange(match(iso3_code, analysis.countries)) %>% # make sure arranged in the right order for future summing 
  mutate(across(-iso3_code, ~ ifelse(iso3_code == benchmark.country.c4, midas.gnb.ddd.benchmark.country.c4, . * midas.gnb.ratio.benchmark.country.c4[[cur_column()]])))
...
# multiply ratio of GNB DDD : CRO by all cro cases 
midas.vrotb.reserve.ddd.exp <- summed.gpos.cases %>%
  mutate(across(-iso3_code, ~ ifelse(iso3_code == benchmark.country.c4, midas.vrotb.ddd.benchmark.country.c4, . * midas.vrotb.ratio.benchmark.country.c4[[cur_column()]])))
...
# Excluding XDR TB - per reviewer revision LPH 01 April 2026
...
typhoid.ddd <- typhoid.cases %>% mutate(across(-iso3_code, ~ . * 10))
...
export(global.exp.ddd, here("results", "abu_estimates", scenario.folder, paste0("global.exp.ddd.", scenario.file, ".RDATA")))
export(exp.total.ddd, here("results", "abu_estimates", scenario.folder, paste0("total.exp.ddd.", scenario.file, ".RDATA")))
export(reserve.ddd.exp, here("results", "abu_estimates", scenario.folder, paste0("reserve.exp.ddd.", scenario.file, ".RDATA")))
export(watch.exp.ddd, here("results", "abu_estimates", scenario.folder, paste0("watch.exp.ddd.", scenario.file, ".RDATA")))
export(access.exp.ddd, here("results", "abu_estimates", scenario.folder, paste0("access.exp.ddd.", scenario.file, ".RDATA")))
export(access.ddd.perc, here("results", "abu_estimates", scenario.folder, paste0("access.ddd.perc.", scenario.file, ".RDATA")))
...
export(total.exp.did, here("results", "abu_estimates", scenario.folder, paste0("total.exp.did.", scenario.file, ".RDATA")))
export(reserve.exp.did, here("results", "abu_estimates", scenario.folder, paste0("reserve.exp.did.", scenario.file, ".RDATA")))
export(watch.exp.did, here("results", "abu_estimates", scenario.folder, paste0("watch.exp.did.", scenario.file, ".RDATA")))
export(access.exp.did, here("results", "abu_estimates", scenario.folder, paste0("access.exp.did.", scenario.file, ".RDATA")))
...
export(get(paste0(scenario.folder, ".summary")), here("results", "abu_estimates", scenario.folder, paste0(scenario.file, ".summary.RDATA")))
